...
def handle_new_comment(payload, installation_token):
    ...
    files = get_pr_files(owner, repo_name, pull_number, installation_token)
    content_list = []
    for file in files:
        file_url = file["contents_url"]
        file_content = get_file_content(file_url, installation_token)
    ...
    if comment_body.lower().startswith("@bot"):
        ...
        if len(comment_body.strip().split(" ")) > 1:
            if comment_body.lower().strip() == "@style approve changes":
                ai_fixed_code_list = []  
                for file in files:
                    content = get_file_content(file["contents_url"], installation_token)
                    language = get_language(file["filename"])
                    if language == "Unknown":
                        res = f"Skipping {file['filename']} as it is an unsupported language\n"
                        continue
                    linter_output = run_linter(content, language)
                    ai_code = analyze_code_anthropic(content, linter_output, language)
                    ai_fixed_code_list.append(ai_code)
