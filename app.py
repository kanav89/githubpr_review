To correct the provided code and remove unused imports, you can use the built-in functionality of Visual Studio Code (VS Code) to organize and remove unused imports. Here’s how you can do it:

1. **Enable the "Organize Imports" feature in VS Code settings:**
   - Open the Command Palette in VS Code by pressing `Ctrl+Shift+P` (Windows/Linux) or `Cmd+Shift+P` (Mac).
   - Type "Preferences: Open Settings (JSON)" and select it.
   - Add or modify the following setting to enable "Organize Imports":
     ```json
     {
...
2. **Run the "Organize Imports" command:**
   - Open the Command Palette again.
   - Type "Organize Imports" and select it.

This will automatically sort and remove unused imports from your code.

Here is a simplified version of your code with unused imports removed and some minor corrections for readability:
...
def handle_new_comment(payload):
    ...
    if comment_body.lower().startswith('@bot'):
        ...
        if len(comment_body.split(' ')) > 1:
            ...
            for file in files:
                flake8_output = check_flake8(get_file_content(file['contents_url'], os.getenv("GITHUB_TOKEN")))
...
This version of your code should be cleaner and more organized. The unused imports have been removed using VS Code's built-in functionality.